...
public class ReferenceFinderWindow : EditorWindow
{
    ...
    private async void RemoveAllUnusedAsset()
    {
        int removeCount = RemoveUnusedAsset();
        int totalRemoveCount = removeCount;
        while (removeCount != 0)
        {
            try
            {
            ...
            removeCount = RemoveUnusedAsset();
            totalRemoveCount += removeCount;
        }

        Debug.Log($"共删除 {totalRemoveCount} 个无效资源");
    }

    private int RemoveUnusedAsset()
    {
        if (m_AssetTreeView == null)
        {
            return 0;
        ...
        IList<TreeViewItem> rowItems = m_AssetTreeView.GetRows();
        int deleteCount = 0;
        for (var i = 0; i < rowItems.Count; i++)
        {
            TreeViewItem data = rowItems[i];
            if (!data.hasChildren)
            {
                AssetViewItem assetViewItem = data as AssetViewItem;
                if (assetViewItem.data.path.EndsWith(".unity") || assetViewItem.data.path.EndsWith(".asset"))
                {
                    continue;
                }

                string assetPath = assetViewItem.data.path;

                bool removeSuccess = AssetDatabase.DeleteAsset(assetPath);
                if (removeSuccess)
                {
                    Debug.Log($"已删除 {rowItems[i].displayName}");
                    deleteCount++;
                }
            }
        }
    ...
    {
        selectedAssetGuid.Sort((x, y) =>
        {
            if (data.assetDict.ContainsKey(x) && data.assetDict.ContainsKey(y))
            {
                ReferenceFinderData.AssetDescription xData = data.assetDict[x];
                ReferenceFinderData.AssetDescription yData = data.assetDict[y];
                return xData.references.Count - yData.references.Count;
            }
    ...
    private AssetViewItem CreateTree(string guid, ref int elementCount, int _depth, Stack<string> stack)
    {
        ...
        if (referenceData.state == ReferenceFinderData.AssetState.MISSING)
        {
            return null;
        }

        if (AssetDatabase.LoadAssetAtPath<UnityEngine.Object>(referenceData.path) == null)
        {
            return null;
        }
